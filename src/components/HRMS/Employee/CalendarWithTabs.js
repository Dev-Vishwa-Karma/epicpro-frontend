...
import { CalendarWithTabService } from '../../../services/CalendarWithTabService';
...
class CalendarWithTabs extends Component {
    ...
    getDepartments = () => {
        CalendarWithTabService.getDepartmentsList()
            .then(data => {
    ...
    fetchData = (url, onSuccessKey) => {
        // This function is now replaced by direct service calls below
    }
    ...
    getAlternateSaturday = async () => {
        ...
        try {
            const data = await CalendarWithTabService.getAlternateSaturdays(now.getFullYear());
            this.setState({
    ...
    loadEmployeeData = () => {
        ...
        if (currentEmployeeId) {
            // Fetch activities
            CalendarWithTabService.getActivities(currentEmployeeId)
                .then(data => {
            ...
            // Fetch reports
            CalendarWithTabService.getReports(currentEmployeeId, startDate, endDate)
                .then(data => {
                    if (data.status === 'success') {
            ...
            // Fetch leaves
            CalendarWithTabService.getLeaves(currentEmployeeId)
                .then(data => {
                    if (data.status === 'success') {
    ...
    fetchEmployeeDetails = (employeeId) => {
        CalendarWithTabService.getEmployeeDetails(employeeId)
            .then((data) => {
                if (data.status === "success") {
                    this.setState(prevState => ({
                        employee: { ...prevState.employee, ...data.data },
    ...
    fetchWorkingHoursReports = (employeeId) => {
        ...
        CalendarWithTabService.getReports(employeeId, startDate, endDate)
            .then((data) => {
                if (data.status === "success") {
                    if (employeeId === '') {
                        this.setState({
    ...
    updateProfile = () => {
        ...
        CalendarWithTabService.saveProfile(employee.id, updatedProfileData)
            .then(data => {
                if (data.status === "success") {
