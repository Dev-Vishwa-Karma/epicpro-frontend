...
import { ViewEmployeeService } from '../../../services/ViewEmployeeService';
...
class ViewEmployee extends Component {
    ...
    toDataURL = async (url) => {
        return ViewEmployeeService.fetchGalleryImageDataUrl(url);
    };
    ...
    handleFileChange = async (event) => {
        ...
        try {
            const file = event.target.files[0];
            const data = await ViewEmployeeService.uploadGalleryImage(this.state.employeeId, file);
    ...
    handleSave = async () => {
        ...
        try {
            const data = await ViewEmployeeService.uploadProfileImage(this.state.employeeId, croppedImage);
            if (data.status === "success") {
    ...
    getEmployeeGallery = (id) => {
        ViewEmployeeService.getEmployeeGallery(id)
            .then(data => {
                if (data.status === 'success') {
    ...
    fetchEmployeeDetails = (employeeId) => {
        ViewEmployeeService.fetchEmployeeDetails(employeeId)
            .then((data) => {
                if (data.status === "success") {
